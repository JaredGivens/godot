...
class JPH_EXPORT FaceChunkShapeSettings final : public ShapeSettings {
public:
	FaceChunkShapeSettings(const ::Ref<SSK::Faces> &faces) :
			mFaces(faces) {}

	virtual ShapeResult Create() const override;
	::Ref<SSK::Faces> mFaces;
};

class JPH_EXPORT FaceChunkShape final : public Shape {
public:
	...
	FaceChunkShape(const FaceChunkShapeSettings &inSettings, ShapeResult &outResult);
	::Ref<SSK::Faces> mFaces;

	virtual bool MustBeStatic() const override { return true; }
	// See Shape::GetLocalBounds
	virtual AABox GetLocalBounds() const override { return AABox(Vec3(), Vec3::sReplicate(SSK::kSize)); }
	// See Shape::GetSubShapeIDBitsRecursive
	virtual uint GetSubShapeIDBitsRecursive() const override { return cSubShapeIDBits; }
	// See Shape::GetInnerRadius
	virtual float GetInnerRadius() const override { return SSK::kSize / 2.0F; }
	...
	inline void DecodeTriangle(uint inFace, uint inTriangle, Vec3 outTriangle[3]) const;
...
private:
	...
	inline SubShapeID EncodeSubShapeID(const SubShapeIDCreator &inCreator, uint inFace, uint inTriangle) const;
	inline void DecodeSubShapeID(const SubShapeID &inSubShapeID, uint &outFace, uint &outTriangle) const;

	static constexpr uint cSubShapeIDBits = 18; // 15 bits for 32 * 32 * 32 and 2 bits for * 3 and 1 bit for face tri
};
