...
String eosg_epic_account_id_to_string(EOS_EpicAccountId accountId) {
    ...
    if (IEOS::get_singleton()->isEOSValid) {
        ERR_PRINT(vformat("[EOSG] eosg_epic_account_id_to_string: Got EOS Result: %s", EOS_EResult_ToString(Result)));
    }
...
String eosg_product_user_id_to_string(EOS_ProductUserId localUserId) {
    ...
    if (IEOS::get_singleton()->isEOSValid) {
        ERR_PRINT(vformat("[EOSG] eosg_product_user_id_to_string: Got EOS Result: %s", EOS_EResult_ToString(Result)));
    }
